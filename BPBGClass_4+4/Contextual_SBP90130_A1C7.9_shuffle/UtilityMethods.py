        ...
        for h in range(self.EPISODE_LENGTH):
         for s in range(self.N_STATES):
            for a in self.ACTIONS[s]:
                ...
                # con_policy  += opt_q[h,s,a]*(max(0, 90-self.C[s][a]) + max(0, self.C[s][a]-130)) # since the cost here is the SBP feedback
                con1_policy  += opt_q[h,s,a]*( max(0, 90.0-self.C1[s][a]) + max(0, self.C1[s][a]-130.0) ) # since the cost here is the SBP feedback
                # con2_policy  += opt_q[h,s,a]*( max(0, 7.9-self.C2[s][a]) + max(0, self.C2[s][a]-7.9)) # since the cost here is the hba1c feedback
                con2_policy  += opt_q[h,s,a]*( max(0, self.C2[s][a]-7.9)) # since the cost here is the hba1c feedback
    ...
    def compute_extended_LP(self, inference=False):
        ...
        for s in range(self.N_STATES):
            ...
            for a in self.ACTIONS[s]:
                ...
                c1_k[s][a] = max(0, 90-(self.C1_hat[s][a] - self.alpha_c1 * self.sbp_confidence[s][a])) + max(0, (self.C1_hat[s][a] + self.alpha_c1 * self.sbp_confidence[s][a]) - 130)
                #c2_k[s][a] = max(0, 7.9-(self.C2_hat[s][a] - self.alpha_c2 * self.hba1c_confidence[s][a])) + max(0, (self.C2_hat[s][a] + self.alpha_c2 * self.hba1c_confidence[s][a]) - 7.9)
                c2_k[s][a] = max(0, (self.C2_hat[s][a] + self.alpha_c2 * self.hba1c_confidence[s][a]) - 7.9)
    ...
    def FiniteHorizon_Policy_evaluation(self, Px, policy, R, C1, C2):
        ...
        for s in range(self.N_STATES):
            ...
            for a in self.ACTIONS[s]:
                ...
                # x += policy[s, self.EPISODE_LENGTH - 1, a]* (max(0, 90-C[s][a]) + max(0, C[s][a]-130)) 
                x1 += policy[s, self.EPISODE_LENGTH - 1, a]* (max(0, 90.0-C1[s][a]) + max(0, C1[s][a]-130.0) ) # sbp
                #x2 += policy[s, self.EPISODE_LENGTH - 1, a]* (max(0, 7.9-C2[s][a]) + max(0, C2[s][a]-7.9)) # hba1c
                x2 += policy[s, self.EPISODE_LENGTH - 1, a]* (max(0, C2[s][a]-7.9)) # hba1c
        ...
        for h in range(self.EPISODE_LENGTH):
            for s in range(self.N_STATES):
                ...
                for a in self.ACTIONS[s]:
                    ...
                    # y += policy[s,h,a]*(max(0, 90-C[s][a]) + max(0, C[s][a]-130))
                    y1 += policy[s,h,a]*(max(0, 90.0-C1[s][a]) + max(0, C1[s][a]-130.0) ) # sbp deviation
                    #y2 += policy[s,h,a]*(max(0, 7.9-C2[s][a]) + max(0, C2[s][a]-7.9))
                    y2 += policy[s,h,a]*( max(0, C2[s][a]-7.9))
